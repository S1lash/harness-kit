...
| `check_portability.py` | Proves every file the kit ships would run the same on Windows, macOS and Linux — the machine-checkable half of `rules/cross-platform.md`. Called by `check_kit.py`; run it alone to see what it checks. | `python3 tools/check_portability.py [--report\|--rules\|--path <p>]` |
